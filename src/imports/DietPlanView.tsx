...
export function DietPlanView() {
  ...
  const loadDietItems = async () => {
    ...
    try {
      ...
      const apiItems = await dietItemsAPI.getAll();
      
      // 2. Load items added from Food Database (LocalStorage)
      const localData = localStorage.getItem('resilio_diet_plan');
      const localItemsRaw = localData ? JSON.parse(localData) : [];
      
      // 3. Map Local items to match the DietItem interface
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-purple-50 border-2 border-purple-600 p-4 rounded-xl">
          <div className="flex items-center gap-2 mb-2">
            <Droplet className="w-5 h-5 text-purple-600" />
            <span className="text-purple-800 font-semibold">Fats</span>
          </div>
          <p className="text-purple-900 text-2xl font-bold">{totalFats}g</p>
        </div>
      </div>
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                      ...
                      <div className="bg-purple-50 p-2 rounded border border-purple-100 text-center">
                        <p className="text-[10px] text-purple-600 font-bold uppercase">Fats</p>
                        <p className="text-purple-900 font-bold">{item.fats}g</p>
                      </div>
                    </div>
                  </div>
